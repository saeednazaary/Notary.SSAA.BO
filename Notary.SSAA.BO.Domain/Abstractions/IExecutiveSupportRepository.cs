...
{
    public interface IExecutiveSupportRepository : IRepository<ExecutiveSupport>
    {
        Task<string> GetMaxReqNo(string scriptorumId, CancellationToken cancellationToken);
        Task<ExecutiveSupportGrid> GetExecutiveSupportGridItems(int pageIndex, int pageSize, ICollection<SearchData> GridSearchInput,
