...
{
    internal sealed class DocumentTypeGroupOneRepository : Repository<DocumentTypeGroup1>, IDocumentTypeGroupOneRepository
    {
        public DocumentTypeGroupOneRepository(ApplicationContext context) : base(context)
        {
        }
        ...
        public async Task<BaseLookupRepositoryObject> GetDocumentTypeLookupItems(int pageIndex, int pageSize, ICollection<SearchData> GridSearchInput, string GlobalSearch,
            SortData gridSortInput, IList<string> selectedItemsIds, List<string> FieldsNotInFilterSearch, DocumentTypegroupOneExtraParams extraParams, CancellationToken cancellationToken, bool isOrderBy = false)
        {
