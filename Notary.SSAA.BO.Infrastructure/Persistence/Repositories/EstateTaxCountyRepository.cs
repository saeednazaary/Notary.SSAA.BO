...
{
    internal class EstateTaxCountyRepository : Repository<EstateTaxCounty>, IEstateTaxCountyRepository
    {
        public EstateTaxCountyRepository(ApplicationContext context) : base(context)
        {
        }

        public async Task<BaseLookupRepositoryObject> GetEstateTaxCountyItems(int pageIndex, int pageSize,ICollection<SearchData> GridSearchInput, string GlobalSearch, 
            SortData gridSortInput, IList<string> selectedItemsIds, List<string> FieldsNotInFilterSearch,bool isOrderBy,string provinceId, CancellationToken cancellationToken)
        {
            BaseLookupRepositoryObject result = new();
            BaseLookupItem lookupFilterItem = new();
            var state = "1";
            var query = TableNoTracking.
                Where(x => x.State == state && x.EstateTaxProvinceId == provinceId);
