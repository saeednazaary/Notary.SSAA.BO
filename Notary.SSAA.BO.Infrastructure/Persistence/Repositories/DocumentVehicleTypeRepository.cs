...
{
    internal class DocumentVehicleTypeRepository : Repository<DocumentVehicleType>, IDocumentVehicleTypeRepository
    {
        public DocumentVehicleTypeRepository(ApplicationContext context) : base(context)
        {
        }

        public async Task<BaseLookupRepositoryObject> GetDocumentVehicleTypeLookupItems(int pageIndex, int pageSize,ICollection<SearchData> GridSearchInput, string GlobalSearch, 
            SortData gridSortInput, IList<string> selectedItemsIds, List<string> FieldsNotInFilterSearch, DocumentVehicleTypeExtraParams extraParams, CancellationToken cancellationToken, bool isOrderBy = false)
        {
            ...
            var query = TableNoTracking.Where(x => x.State == State.Valid);
            if (extraParams is not null)
            {
                if (extraParams.MadeInIran!=null)
                    query = query.Where(x => x.MadeInIran == extraParams.MadeInIran.ToYesNo());
            }
