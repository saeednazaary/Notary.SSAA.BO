...
{
    public class CostTypeTypeRepository : Repository<CostType>, ICostTypeRepository
    {
        public CostTypeTypeRepository(ApplicationContext context) : base(context)
        {
        }

        public async Task<BaseLookupRepositoryObject> GetCostTypeLookupItems(int pageIndex, int pageSize, ICollection<SearchData> GridSearchInput, string GlobalSearch, SortData gridSortInput, IList<string> selectedItemsIds, List<string> FieldsNotInFilterSearch, DocumentCostTypeLookupExtraParams extraParams, CancellationToken cancellationToken, bool isOrderBy = false)
        {
            ...
            {
                if (extraParams.CostTypesId.Count>0)
                    initialQuery = initialQuery.Where(x => !extraParams.CostTypesId.Contains( x.Id));
            }
        ...
        public  Task<CostType> GetIdByCode(string code)
        {
            return  TableNoTracking.FirstOrDefaultAsync(x => x.Code == code);
        }
    }
}
