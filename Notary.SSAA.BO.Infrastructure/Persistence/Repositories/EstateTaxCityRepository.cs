...
{
    internal class EstateTaxCityRepository : Repository<EstateTaxCity>, IEstateTaxCityRepository
    {
        public EstateTaxCityRepository(ApplicationContext context) : base(context)
        {
        }

        public async Task<BaseLookupRepositoryObject> GetEstateTaxCityItems(int pageIndex, int pageSize,ICollection<SearchData> GridSearchInput, string GlobalSearch, 
            SortData gridSortInput, IList<string> selectedItemsIds, List<string> FieldsNotInFilterSearch,bool isOrderBy,string countyId, CancellationToken cancellationToken)
        {
            BaseLookupRepositoryObject result = new();
            BaseLookupItem lookupFilterItem = new();
            var state = "1";
            var query = TableNoTracking.
                Where(x => x.State == state && x.EstateTaxCountyId == countyId);
