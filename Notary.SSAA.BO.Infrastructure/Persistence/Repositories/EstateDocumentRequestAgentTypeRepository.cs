...
{
    public class EstateDocumentRequestAgentTypeRepository : Repository<AgentType>, IEstateDocumentRequestAgentTypeRepository
    {
        public EstateDocumentRequestAgentTypeRepository(ApplicationContext dbContext) : base(dbContext)
        {
        }

        public async Task<BaseLookupRepositoryObject> GetEstateDocumentAgentTypeItems(int pageIndex, int pageSize, ICollection<SearchData> gridSearchInput, string globalSearch, SortData gridSortInput, IList<string> selectedItemsIds, List<string> fieldsNotInFilterSearch, bool isOrderBy, CancellationToken cancellationToken)
        {
