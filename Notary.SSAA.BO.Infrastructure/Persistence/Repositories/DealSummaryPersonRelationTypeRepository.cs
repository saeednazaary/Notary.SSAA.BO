...
{
    public class DealSummaryPersonRelationTypeRepository:Repository<DealsummaryPersonRelateType>,IDealSummaryPersonRelationTypeRepository
    {
        public DealSummaryPersonRelationTypeRepository(ApplicationContext dbContext) : base(dbContext)
        {

        }
        
        
        public async Task<BaseLookupRepositoryObject> GetDealSummaryPersonRelationTypeForTaxInquiryGridItems(int pageIndex, int pageSize, ICollection<SearchData> gridSearchInput, string globalSearch, SortData gridSortInput, IList<string> selectedItemsIds, List<string> fieldsNotInFilterSearch,bool isOrderBy, CancellationToken cancellationToken)
        {
            BaseLookupRepositoryObject result = new ();
            var state = "1";
            var query = TableNoTracking.
                Where(x => x.State == state && (x.Code=="101" || x.Code=="102"));
