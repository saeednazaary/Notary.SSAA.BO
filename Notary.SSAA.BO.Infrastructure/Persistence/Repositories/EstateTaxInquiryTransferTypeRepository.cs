...
{
    internal class EstateTaxInquiryTransferTypeRepository : Repository<EstateTaxInquiryTransferType>, IEstateTaxInquiryTransferTypeRepository
    {
        public EstateTaxInquiryTransferTypeRepository(ApplicationContext context) : base(context)
        {
        }

        public async Task<BaseLookupRepositoryObject> GetEstateTaxInquiryTransferTypeItems(int pageIndex, int pageSize, ICollection<SearchData> gridSearchInput, string globalSearch, SortData gridSortInput, IList<string> selectedItemsIds, List<string> fieldsNotInFilterSearch, bool isOrderBy, CancellationToken cancellationToken)
        {
