...
{
    internal class DocumentVehicleSystemRepository : Repository<DocumentVehicleSystem>, IDocumentVehicleSystemRepository
    {
        public DocumentVehicleSystemRepository(ApplicationContext context) : base(context)
        {
        }

        public async Task<BaseLookupRepositoryObject> GetDocumentVehicleSystemLookupItems(int pageIndex, int pageSize,ICollection<SearchData> GridSearchInput, string GlobalSearch, 
            SortData gridSortInput, IList<string> selectedItemsIds, List<string> FieldsNotInFilterSearch, DocumentVehicleSystemExtraParams extraParams, CancellationToken cancellationToken, bool isOrderBy = false)
        {
            ...
            {
                if (!extraParams.DocumentVehicleTypeId.IsNullOrWhiteSpace())
                    initialQuery = initialQuery.Where(x => x.DocumentVehicleTypeId == extraParams.DocumentVehicleTypeId);
            }
