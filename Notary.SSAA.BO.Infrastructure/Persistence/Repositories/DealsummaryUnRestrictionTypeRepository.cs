...
{
    public class DealsummaryUnRestrictionTypeRepository:Repository<DealsummaryUnrestrictionType>,IDealSummaryUnRestrictionTypeRepository
    {
        public DealsummaryUnRestrictionTypeRepository(ApplicationContext context) : base(context)
        {
        }
        
        public async Task<BaseLookupRepositoryObject> GetDealSummaryUnRestrictionTypeItems(int pageIndex, int pageSize, ICollection<SearchData> gridSearchInput, string globalSearch, SortData gridSortInput, IList<string> selectedItemsIds, List<string> fieldsNotInFilterSearch, bool isOrderBy, CancellationToken cancellationToken)
        {
            ...
            {
                item.Title = item.Title.NormalizeTextChars(false);
            }
