...
{
    public class EstateDocumentCurrentTypeRepository:Repository<EstateDocumentCurrentType>,IEstateDocumentCurrentTypeRepository
    {
        public EstateDocumentCurrentTypeRepository(ApplicationContext dbContext) : base(dbContext)
        {
        }

        public async Task<BaseLookupRepositoryObject> GetEstateDocumentCurrentTypeItems(int pageIndex, int pageSize, ICollection<SearchData> gridSearchInput, string globalSearch, SortData gridSortInput, IList<string> selectedItemsIds, List<string> fieldsNotInFilterSearch, bool isOrderBy, CancellationToken cancellationToken)
        {
