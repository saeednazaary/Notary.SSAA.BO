...
{
    public class ElzamArtSixOrganRepository : Repository<SsrArticle6Organ>, IElzamArtSixOrganRepository
    {
        public ElzamArtSixOrganRepository(ApplicationContext dbContext) : base(dbContext)
        {

        }

        public async Task<BaseLookupRepositoryObject> GetElzamArtSixOrganLookupItems(int pageIndex, int pageSize, ICollection<SearchData> GridSearchInput, string GlobalSearch,
            SortData gridSortInput, IList<string> selectedItemsIds, List<string> FieldsNotInFilterSearch, CancellationToken cancellationToken, bool isOrderBy = false)
        {
            ...
            if (!string.IsNullOrWhiteSpace(filterQueryString))
                initialQuery = initialQuery.Where(filterQueryString.ArabicTopersian());
