...
{
    internal class EstateTaxInquiryDocumentRequestTypeRepository : Repository<EstateTaxInquiryDocumentRequestType>, IEstateTaxInquiryDocumentRequestTypeRepository
    {
        public EstateTaxInquiryDocumentRequestTypeRepository(ApplicationContext context) : base(context)
        {
        }

        public async Task<BaseLookupRepositoryObject> GetEstateTaxInquiryDocumentRequestTypeItems(int pageIndex, int pageSize, ICollection<SearchData> gridSearchInput, string globalSearch, SortData gridSortInput, IList<string> selectedItemsIds, List<string> fieldsNotInFilterSearch, bool isOrderBy, CancellationToken cancellationToken)
        {
