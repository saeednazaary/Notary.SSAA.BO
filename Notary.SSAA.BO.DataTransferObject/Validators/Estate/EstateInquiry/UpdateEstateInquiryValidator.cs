...
{
    public class UpdateEstateInquiryValidator : AbstractValidator<UpdateEstateInquiryCommand>
    {
        public UpdateEstateInquiryValidator()
        {
            //RuleFor(x => x.IsValid)
            //  .Must(x => x == true).WithMessage("وضعیت استعلام نا معتبر است");

            RuleFor(x => x.IsDirty)
                .Must(x => x == true).WithMessage("وضعیت استعلام ویرایش نمیباشد");

            RuleFor(x => x.IsNew)
                .Must(x => x == false).WithMessage("وضعیت استعلام جدید میباشد");

            RuleFor(x => x.IsDelete)
                .Must(x => x == false).WithMessage("وضعیت استعلام حذف شده میباشد");
            RuleFor(x => x.InqId).NotEmpty().WithMessage("شناسه استعلام نمی تواند خالی باشد");
            RuleFor(x => x.InqInquiryNo).NotEmpty().WithMessage("شماره استعلام اجباری می باشد");
            RuleFor(x => x.InqInquiryDate).NotEmpty().WithMessage("تاریخ  استعلام اجباری می باشد");
