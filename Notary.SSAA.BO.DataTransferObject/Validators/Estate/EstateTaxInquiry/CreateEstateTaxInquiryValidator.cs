...
{
    public class CreateEstateTaxInquiryValidator : AbstractValidator<CreateEstateTaxInquiryCommand>
    {
        public CreateEstateTaxInquiryValidator()
        {
            ...
            RuleFor(x => x.TheInquiryOwner)
             .Must(x =>
             {
                 return x.IsNew && !x.IsDelete;
             }).When(x => x.TheInquiryOwner != null).WithMessage("اطلاعات مالک باید در وضعیت  جدید باشد");

            RuleFor(x => x.TheInquiryBuyersList)
           .Must(y =>
           {
               bool bv = false;
               foreach (var buyer in y)
               {
                   if (buyer.IsNew && !buyer.IsDelete)
                       bv = true;
               }
               return bv;
           })
           .When(x => x.TheInquiryBuyersList != null && x.TheInquiryBuyersList.Count > 0).WithMessage("لیست خریداران فاقد رکورد در وضعیت جدید و حذف نشده می باشد");

            RuleFor(x => x.TheInquiryEstateAttachList)
            .Must(y =>
            {
                bool bv = false;
                foreach (var attach in y)
                {
                    if (attach.IsNew && !attach.IsDelete)
                        bv=true;
                }
                return bv;
            }).When(y => y.TheInquiryEstateAttachList != null && y.TheInquiryEstateAttachList.Count > 0).WithMessage("لیست منضمات ملک فاقد رکورد در وضعیت جدید و حذف نشده میباشد");

            RuleFor(x => x.InquiryId).Empty().WithMessage("شناسه استعلام باید خالی باشد");
            ...
            RuleFor(x => x.TheInquiryOwner.PersonType).NotEmpty().When(x => x.TheInquiryOwner != null && x.TheInquiryOwner.IsNew  && !x.TheInquiryOwner.IsDelete).WithMessage("انتخاب نوع شخص (حقیقی/حقوقی) برای مالک اجباری می باشد");
            RuleFor(v => v.TheInquiryOwner.PersonNationalityCode).NotEmpty().When(x => x.TheInquiryOwner != null && x.TheInquiryOwner.IsNew && !x.TheInquiryOwner.IsDelete).WithMessage("شماره ملی/شناسه ملی مالک نمی تواند خالی باشد");
            ...
            RuleFor(v => v.TheInquiryOwner.PersonMobileNo).NotEmpty().When(x => x.TheInquiryOwner != null && x.TheInquiryOwner.IsNew && !x.TheInquiryOwner.IsDelete).WithMessage("شماره تلفن همراه مالک نمی تواند خالی باشد");
            RuleFor(v => v.TheInquiryOwner.PersonTel).NotEmpty().When(x => x.TheInquiryOwner != null && x.TheInquiryOwner.IsNew && !x.TheInquiryOwner.IsDelete).WithMessage("شماره تلفن تماس مالک نمی تواند خالی باشد");
            RuleFor(v => v.TheInquiryOwner.PersonName).NotEmpty().When(x => x.TheInquiryOwner != null && x.TheInquiryOwner.IsNew  && !x.TheInquiryOwner.IsDelete).WithMessage("نام مالک نمی تواند خالی باشد");
            RuleFor(v => v.TheInquiryOwner.PersonFamily).NotEmpty().When(x => x.TheInquiryOwner != null && x.TheInquiryOwner.IsNew  && !x.TheInquiryOwner.IsDelete && x.TheInquiryOwner.PersonType == EstateConstant.PersonTypeConstant.RealPerson).WithMessage("نام خانوادگی مالک نمی تواند خالی باشد");
            ...
            {
                return IsMatch(@"^(\d{10})$", x);
            }).When(x => x.TheInquiryOwner != null && x.TheInquiryOwner.IsNew  && !x.TheInquiryOwner.IsDelete && !string.IsNullOrWhiteSpace(x.TheInquiryOwner.PersonPostalCode)).WithMessage("کد پستی محل سکونت مالک اشتباه می باشد");
            RuleFor(v => v.TheInquiryOwner.PersonMobileNo).Must(x =>
            {
                return ValidatorHelper.BeValidMobileNumber(x);
            }).When(x => x.TheInquiryOwner != null && x.TheInquiryOwner.IsNew  && !x.TheInquiryOwner.IsDelete && !string.IsNullOrWhiteSpace(x.TheInquiryOwner.PersonMobileNo)).WithMessage(" شماره موبایل مالک اشتباه می باشد");
            RuleFor(v => v.TheInquiryOwner.PersonBirthDate).Must(x =>
            {               
                return ValidatorHelper.BeValidPersianDate(x);
            }).When(x => x.TheInquiryOwner != null && x.TheInquiryOwner.IsNew  && !x.TheInquiryOwner.IsDelete && !string.IsNullOrWhiteSpace(x.TheInquiryOwner.PersonBirthDate)).WithMessage(" تاریخ تولد/ثبت مالک اشتباه می باشد");
