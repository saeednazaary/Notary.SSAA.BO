    ...
    public class CreateDocumentCommandHandler : BaseCommandHandler<CreateDocumentCommand, ApiResult>
    {
        ...
        private void BusinessValidation(CreateDocumentCommand request)
        {
            if (string.IsNullOrEmpty(request.RequestId))
            {
                ...
                {
                    apiResult.message.Add("زمانیکه درخواست  براساس حکم دادگاه است، ایجاد حکم دادگاه اجباری است.");
                }
                else if (request.IsRequestBasedJudgment == true && request.DocumentInfoJudgment != null)
                {
                    string documentJudgmentTypeIdTmp = request.DocumentInfoJudgment.DocumentJudgmentTypeId != null ? request.DocumentInfoJudgment.DocumentJudgmentTypeId.FirstOrDefault() : string.Empty;
                    ...
                    {
                        apiResult.message.Add(" .فیلد نوع حکم اجباری است");
                    }
                    if (string.IsNullOrEmpty(request.DocumentInfoJudgment.IssueDate))
                    {
                        apiResult.message.Add(" فیلد تاریخ حکم اجباری است.");
                    }
                    if (string.IsNullOrEmpty(request.DocumentInfoJudgment.IssueNo))
                    {
                        apiResult.message.Add(" فیلد شماره حکم اجباری است.");
                    }
                    if (string.IsNullOrEmpty(request.DocumentInfoJudgment.IssuerName))
                    {
                        apiResult.message.Add(" فیلد  صادرکننده حکم اجباری است.");
                    }
                    if (string.IsNullOrEmpty(request.DocumentInfoJudgment.CaseClassifyNo))
                    {
                        apiResult.message.Add(" فیلد شماره کلاسه حکم اجباری است.");
                    }
                }
                foreach (DocumentRelatedPersonViewModel item in request.DocumentRelatedPeople)
                {
                    if (item.IsNew)
                    {
                        ...
                        {
                            apiResult.message.AddRange(validateRelatedAgentDocumentIssuer);
                        }
                        ...
                        if (item.RelatedAgentTypeId.First() == "1")
                        {
                            ...
                            {
                                apiResult.message.Add("طول شماره وکالتنامه نباید بیشتر از 50 کاراکتر باشد.");
                            }
                            ...
                            if (item.IsRelatedAgentDocumentAbroad)
                            {
                                ...
                                {
                                    apiResult.message.Add("فیلد کشور اجباری میباشد .");
                                }
                            ...
                            else
                            {
                                ...
                                {
                                    apiResult.message.Add("فیلد دفترخانه اجباری میباشد .");
                                }

                                if (item.IsRelatedDocumentInSSAR)
                                {
                                    if (item.RelatedAgentDocumentDate.GreaterThanEqual("1392/06/26"))
                                    {
                                        apiResult.message.Add("تاریخ وکالتنامه غیر مجاز است .");
                                    }
                                }
                                else
                                {
                                    if (item.RelatedAgentDocumentDate.LessThanEqual("1392/06/26"))
                                    {
                                        apiResult.message.Add("تاریخ وکالتنامه غیر مجاز است .");
                                    }
                                }
                        ...
                        if (item.RelatedAgentTypeId.First() != "03" && item.RelatedAgentTypeId.First() != "01")
                        {
                            if (!ValidatorHelper.BeValidPersianDate(item.RelatedAgentDocumentDate))
                            {
                                apiResult.message.Add("مقدار تاریخ وکالتنامه غیر مجاز است ");
                            ...
                            {
                                apiResult.message.Add("فیلد شماره وکالتنامه نباید بیشتر از 50 کاراکتر باشد.");
                            }
                            else if (string.IsNullOrWhiteSpace(item.RelatedAgentDocumentNo))
                            {
                                apiResult.message.Add("فیلد شماره وکالتنامه اجباری است.");
                            }
                        ...
                        if (!masterEntity.DocumentPeople.Any(x => x.Id == item.MainPersonId.First().ToGuid()))
                        {

                            apiResult.message.Add("شخص اصیل شخص وابسته حذف شده است");
                        }

                        if (!masterEntity.DocumentPeople.Any(x => x.Id == item.RelatedAgentPersonId.First().ToGuid()))
                        {
                ...
                foreach (DocumentPersonViewModel item in request.DocumentPeople)
                {
                    if (item.IsNew)
                    {
                        string documentType = request.DocumentTypeId.First();
                        //اجرائیه
                        if (documentType == "007")
                        {
