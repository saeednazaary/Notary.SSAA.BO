...
using Notary.SSAA.BO.DataTransferObject.ServiceInputs.BO.Exordium;
...
{
    internal class UpdateDocumentPaymentStateCommandHandler : BaseCommandHandler<UpdateDocumentPaymentStateCommand, ApiResult<UpdateDocumentPaymentStateViewModel>>
    {
        private Domain.Entities.Document masterEntity;
        private readonly IDocumentRepository _documentRepository;
        private ApiResult<UpdateDocumentPaymentStateViewModel> apiResult;
        ...
        public UpdateDocumentPaymentStateCommandHandler(IMediator mediator, IUserService userService, IDocumentRepository documentRepository, IRepository<DocumentPayment> documentPaymentRepository,
        ...
        protected override bool HasAccess(UpdateDocumentPaymentStateCommand request, IList<string> userRoles)
        {
        ...
        protected override async Task<ApiResult<UpdateDocumentPaymentStateViewModel>> ExecuteAsync(UpdateDocumentPaymentStateCommand request, CancellationToken cancellationToken)
        {
                ...
                if (paymentInquiryResult.IsSuccess)
                {
                    if (paymentInquiryResult.Data.State == PaymentInquiryResult.Paid.ToAssignedValue())
                    {
                        ...
                        if (documentPayment == null)
                        {
                            ...
                            DocumentPaymentMapper.MapToInquiryPayment(ref documentPayment, paymentInquiryResult.Data, masterEntity.Id, Guid.NewGuid(), ScriptoriumId);
                            await _documentPaymentRepository.AddAsync(documentPayment, cancellationToken);
                        }
                        else
                        {
                            DocumentPaymentMapper.MapToInquiryPayment(ref documentPayment, paymentInquiryResult.Data, masterEntity.Id, documentPayment.Id, ScriptoriumId);
        ...
        private async Task<ApiResult<UpdateDocumentPaymentStateViewModel>> CalculateCostForRemoteDocument(CancellationToken cancellationToken)
        {
