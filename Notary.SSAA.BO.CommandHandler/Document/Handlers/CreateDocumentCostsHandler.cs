    ...
    public class CreateDocumentCostsHandler : BaseCommandHandler<CreateDocumentCostsCommand, ApiResult>
    {
        ...
        private async Task<ApiResult<DocumentCostsViewModel>> GetEpaymentDocumentCosts(Domain.Entities.Document document, CancellationToken cancellationToken)
        {
            ...
            if (haghTahrir != null)
            {
                ...
                EpaymentCostCalculatorDetail Tax = EpaymentCosts.Data.EpaymentCostCalculatorDetailList.Where(x => x.CostTypeId == "9").FirstOrDefault();
                haghTahrir.Price = haghTahrir.Price.GetValueOrDefault() - ((TenPercent != null ? TenPercent.Price.GetValueOrDefault() : 0) + (Tax != null ? Tax.Price.GetValueOrDefault() : 0));
