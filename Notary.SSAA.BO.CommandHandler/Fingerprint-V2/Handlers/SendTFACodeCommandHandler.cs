...
{
    public class SendTFACodeCommandHandler : BaseCommandHandler<SendTFACodeV2Command, ApiResult>
    {
        ...
        protected override async Task<ApiResult> ExecuteAsync(SendTFACodeV2Command request, CancellationToken cancellationToken)
        {
            ...
            if (masterEntity is not null)
            {
                if (masterEntity.TfaSendDate.IsNullOrWhiteSpace() && masterEntity.TfaSendTime.IsNullOrWhiteSpace())
                {
                    ...
                    masterEntity.TfaState = null;
                    masterEntity.TfaValidateDate = null;
                    masterEntity.TfaValidateTime = null;
                    await _personFingerprintRepository.UpdateAsync(masterEntity, cancellationToken);

                    SendSmsFromSabtServiceInput sendSMSServiceInput = new();
                    sendSMSServiceInput.Message = string.Format(@"درخواستی بنام {0} در {1} در حال تنظیم است. درصورت موافقت کد اعتبارسنجی را جهت ادامه روند اعلام فرمایید.
کد اعتبارسنجی: {2}", masterEntity.NameFamily, _userService.UserApplicationContext.BranchAccess.BranchName, masterEntity.TfaValue);
                    sendSMSServiceInput.Recipient = masterEntity.TfaMobileNo;
                    sendSMSServiceInput.ClientId = ExternalServiceConstant.ClientId.SendOTPFingerprint;
                    sendSMSServiceInput.MainObjectId = masterEntity.Id.ToString();
                    var smsRes=await _mediator.Send(sendSMSServiceInput, cancellationToken);
                    apiResult.message.Add(JsonConvert.SerializeObject(smsRes));
                    // var sendMessageResult = await _httpExternalServiceCaller.CallExternalServicePostApiAsync<ApiResult<SMSServiceViewModel>, SendSMSServiceInput>(new HttpExternalServiceRequest<SendSMSServiceInput>
...
کد اعتبارسنجی: {2}", masterEntity.NameFamily, _userService.UserApplicationContext.BranchAccess.BranchName, masterEntity.TfaValue);
        ...
        protected override bool HasAccess(SendTFACodeV2Command request, IList<string> userRoles)
        {
