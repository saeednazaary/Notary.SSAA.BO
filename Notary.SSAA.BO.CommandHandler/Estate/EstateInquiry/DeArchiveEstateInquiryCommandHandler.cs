            ...
            if (apiResult.IsSuccess)
            {
                if (masterEntity != null)
                {
                    ...
                    if (await _configurationParameterHelper.IsEstateInquiryRealSendEnabled(cancellationToken))
                    {
                        ...
                        var syncResult = await externalServiceHelper.UnArchiveEstateInquiryInSabtTerminalServer(cancellationToken);
                        if (!string.IsNullOrEmpty(syncResult))
                        {
                            masterEntity.WorkflowStatesId = prevStatus;
                            masterEntity.Timestamp = prevTimestamp;
                            _estateInquiryRepository.Update(masterEntity);

                            apiResult.IsSuccess = false;
                            apiResult.message.Add("به علت عدم امکان برقراری ارتباط با  سرویس دهنده سازمان ثبت یا بروز خطا  در آن  ، امکان ثبت خروج از وضعیت بایگانی وجود ندارد");
