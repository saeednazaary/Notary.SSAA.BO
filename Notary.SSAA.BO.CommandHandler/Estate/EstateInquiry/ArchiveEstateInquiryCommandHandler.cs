            ...
            if (apiResult.IsSuccess)
            {
                if (masterEntity != null)
                {
                    ...
                    if (await _configurationParameterHelper.IsEstateInquiryRealSendEnabled(cancellationToken))
                    {
                        ...
                        var syncResult = await externalServiceHelper.ArchiveEstateInquiryInSabtTerminalServer(cancellationToken);
                        ...
                        if (!string.IsNullOrEmpty(syncResult))
                        {
                            ...
                            apiResult.message.Add("به علت عدم امکان برقراری ارتباط با  سرویس دهنده سازمان ثبت یا بروز خطا  در آن  ، امکان بایگانی شدن استعلام وجود ندارد");
