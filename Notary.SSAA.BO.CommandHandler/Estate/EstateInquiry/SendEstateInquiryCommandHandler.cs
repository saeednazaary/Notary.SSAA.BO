...
namespace Notary.SSAA.BO.CommandHandler.Estate.EstateInquiry
{
    public class SendEstateInquiryCommandHandler : BaseCommandHandler<SendEstateInquiryCommand, ApiResult>
    {
                    ...
                    if (await _configurationParameterHelper.IsEstateInquiryRealSendEnabled(cancellationToken))
                    {
                        ...
                        if (!string.IsNullOrWhiteSpace(sendResult))
                        {
                            ...
                            if (needToRollback)
                            {
                                ...
                                if (masterEntity.WorkflowStatesId == EstateConstant.EstateInquiryStates.NotSended)
                                    await externalServiceHelper.DeleteEstateInquiryInSabtTerminalServerByService(cancellationToken);
                                else
                                    await externalServiceHelper.RollbackEstateInquirySendStatusInSabtTerminalServerByService(cancellationToken);
                            }
        ...
        private async Task<string> SendToSabtEstateSystemByService(CancellationToken cancellationToken)
        {
            ...
            var syncResult = await externalServiceHelper.SendEstateInquiryToSabtTerminalServerByService(cancellationToken);
            if (string.IsNullOrWhiteSpace(syncResult))
            {
                needToRollback = true;
                var result = await externalServiceHelper.SendEstateInquiryToSabtEstateSystemByService(cancellationToken);
                return result;
