...
namespace Notary.SSAA.BO.CommandHandler.Estate.LegacySystem
{
    public class LegacySystemEstateTaxInquiryCommandHandler : BaseExternalCommandHandler<EstateTaxInquiryLegacySystemCommand, ExternalApiResult>
    {
        ...
        protected async override Task<ExternalApiResult> ExecuteAsync(EstateTaxInquiryLegacySystemCommand request, CancellationToken cancellationToken)
        {
            ...
            ExternalApiResult apiResult = new() { ResCode = "1", ResMessage = "عملیات با موفقیت انجام شد" };
            ...
            try
            {
                ...
                foreach (var data in list)
                {
                    ...
                    {
                        var estatetaxInquiryId = await SaveEstateTaxInquiry(data, commitChanges,request.UpdateDataOnly, cancellationToken);
                        ...
                        await SendSMS(estatetaxInquiryId.ToString(), cancellationToken);
                    }
        ...
        {
            var idValue = data.FieldValues.Where(fv => fv.FieldName.Equals("estestateinquiryid", StringComparison.OrdinalIgnoreCase)).First().Value.ToString();
        ...
        private async Task<Guid?> SaveEstateTaxInquiry(EntityData data, bool commitChanges,bool updateDataOnly, CancellationToken cancellationToken)
        {
            SSAA.BO.Domain.Entities.EstateTaxInquiry estateTaxInquiry = null;
            var idValue = "";
            var trackingCodeValue = "";
            var idData=data.FieldValues.Where(fv => fv.FieldName.Equals("id", StringComparison.OrdinalIgnoreCase)).FirstOrDefault();
            var trackingCodeData = data.FieldValues.Where(fv => fv.FieldName.Equals("trackingcode", StringComparison.OrdinalIgnoreCase)).FirstOrDefault();
            if (idData != null)
            {
            ...
            else if (trackingCodeData != null)
            {
                trackingCodeValue = trackingCodeData.Value.ToString();
                estateTaxInquiry = await _estateTaxInquiryRepository.GetAsync(x => x.TrackingCode == trackingCodeValue, cancellationToken);
            }
            bool isNew = false;
            ...
            var workflowState = await SetProperties(estateTaxInquiry, data, estateInquiry.ScriptoriumId, cancellationToken);
            ...
            if(hasPerson)
            {
                foreach (var person in PersonList)
                {
                    ...
                    var p = SetProperties(taxInquiryPerson, person,  isLegacyInquiry, estateInquiry.ScriptoriumId);
                    if (p != null)
                    {
            ...
            foreach(var attach in attachList)
            {
                ...
                var a = await SetProperties(taxInquiryAttach, attach,  isLegacyInquiry, estateInquiry.ScriptoriumId,cancellationToken);
                if (a != null)
                {
            ...
            foreach (var file in fileList)
            {
                ...
                var a =  SetProperties(taxInquiryFile, file, isLegacyInquiry, estateInquiry.ScriptoriumId);
                if (a != null)
                {                    
                    a.EstateTaxInquiryId = estateTaxInquiry.Id;
        ...
        private static EstateTaxInquiryFile SetProperties(EstateTaxInquiryFile estateTaxInquiryFile, EntityData file, bool isLegacyInquiry, string scriptoriumId)
        {
            ...
            estateTaxInquiryFile.ScriptoriumId = scriptoriumId;
        ...
        private EstateTaxInquiryPerson SetProperties(EstateTaxInquiryPerson estateTaxInquiryPerson, EntityData Person, bool isLegacyInquiry, string scriptoriumId)
        {
            ...
            estateTaxInquiryPerson.ScriptoriumId = scriptoriumId;
            ...
            estateTaxInquiryPerson.RoleType= GetValue<bool?>(Person, "RoleType");
            if (isNewPersonObject)
                return estateTaxInquiryPerson;
            return null;
        }
        private async Task<EstateTaxInquiryAttach> SetProperties(EstateTaxInquiryAttach estateTaxInquiryAttach, EntityData attachData, bool isLegacyInquiry, string scriptoriumId,CancellationToken cancellationToken)
        {            
            ...
            estateTaxInquiryAttach.ScriptoriumId = scriptoriumId;
        ...
        {            
            ...
            estateTaxInquiry.ScriptoriumId = scriptoriumId;
        ...
        private async Task SendSMS(string estateTaxInquiryId, CancellationToken cancellationToken)
        {
            ...
            if (!string.IsNullOrWhiteSpace(person.MobileNo) && person.MobileNoState == EstateConstant.BooleanConstant.True)
            {
                ...
                var msg = "";
                string msg1 = "مالک محترم" + Environment.NewLine + "استعلام مالیاتی ملک شما با شناسه یکتا سند مالکیت {0} در تاریخ {1} توسط واحد سازمان امور مالیاتی به {2} پاسخ داده شد.";
                string msg2 = "مالک محترم" + Environment.NewLine + "استعلام مالیاتی ملک شما با پلک اصلی  {0} و فرعی {1} در تاریخ {2} توسط واحد سازمان امور مالیاتی به {3} پاسخ داده شد.";
