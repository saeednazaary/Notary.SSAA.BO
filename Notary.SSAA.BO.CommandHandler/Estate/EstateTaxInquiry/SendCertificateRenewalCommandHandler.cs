...
{
    public class SendCertificateRenewalCommandHandler : BaseCommandHandler<SendCertificateRenewalCommand, ApiResult>
    {
        ...
        protected override bool HasAccess(SendCertificateRenewalCommand request, IList<string> userRoles)
        {
            return userRoles.Contains(RoleConstants.Sardaftar) || userRoles.Contains(RoleConstants.Daftaryar) || userRoles.Contains(RoleConstants.SanadNevis);
        }
        protected override async Task<ApiResult> ExecuteAsync(SendCertificateRenewalCommand request, CancellationToken cancellationToken)
        {
            ...
            if (apiResult.IsSuccess)
            {
                try
                {
                    var sendResult = await externalServiceHelper.SendEstateTaxInquiry(request.EstateTaxInquiryId, "02", cancellationToken);
        ...
        private async Task BusinessValidation(SendCertificateRenewalCommand request, CancellationToken cancellationToken)
        {
