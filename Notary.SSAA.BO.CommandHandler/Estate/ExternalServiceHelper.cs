...
namespace Notary.SSAA.BO.CommandHandler.Estate
{
    public class ExternalServiceHelper
    {
        ...
        private DataTransferReceiveServiceInput RollbackEstateInquirySendStatusCommands(string sendingStatus)
        {
            ...
            {
                input = this.GetDeleteEstateInquiryCommands();
                return input;
            }
            else
            {
                EntityData updateInquiry = new();
                updateInquiry.CommandType =  2;
                updateInquiry.EntityName = "ESTESTATEINQUIRY";
                updateInquiry.OrderNo = orderNo;
                updateInquiry.Fields.Add(new Field() { Length = 32, Type = "varchar2", Name = "ID" });
                updateInquiry.Fields.Add(new Field() { Length = 1, Type = "varchar2", Name = "SENDINGSTATUS" });
                updateInquiry.FieldValues.Add(new FieldValue() { FieldName = "ID", Value = !string.IsNullOrWhiteSpace(EstateInquiry.LegacyId) ? EstateInquiry.LegacyId : EstateInquiry.Id.ToString().Replace("-", "").Replace("_", "").ToUpper() });
                updateInquiry.FieldValues.Add(new FieldValue() { FieldName = "SENDINGSTATUS", Value = sendingStatus });
                input.EntityList.Add(updateInquiry);
                return input;
            }
            
        }
        public async Task<string> SendEstateInquiryToSabtTerminalServerByService(CancellationToken cancellationToken)
        {
        ...
        public async Task<string> ArchiveEstateInquiryInSabtTerminalServer(CancellationToken cancellationToken)
        {
            if (EstateInquiry == null) return "استعلام مرتبط مشخص نیست";
            var input = new DataTransferReceiveServiceInput();
            input.ReceiverCmsorganizationId = estateInquiryScriptoriumOrganization.LegacyId;
            input.SenderCmsorganizationId = estateInquiryScriptoriumOrganization.LegacyId;
            input.ReceiverSubsystemName = 2;
            input.RequestDateTime = _dateTimeService.CurrentPersianDateTime;
            input.EntityList = new List<EntityData>()
            {
                 new EntityData()
                 {
                      CommandType=2,
                      EntityName="ESTESTATEINQUIRY",
                      Fields=new List<Field>()
                      {
                         new Field(){ Length=1, Name="SendingStatus", Type="VARCHAR2"},
                         new Field(){ Length=32, Name="Id", Type="VARCHAR2"}
                      },
                      OrderNo=1,
                      FieldValues=new List<FieldValue>()
                      {
                         new FieldValue(){ FieldName="SendingStatus", Value="4"},
                         new FieldValue(){ FieldName="Id", Value= string.IsNullOrWhiteSpace(EstateInquiry.LegacyId) ? EstateInquiry.Id.ToString().Replace("-","").Replace("_","").ToUpper() : EstateInquiry.LegacyId }
                      }
                 }
            };
            ...
            if (output.IsSuccess)
            {
                if (output.Data != null && output.Data.Successful)
                    return "";
                else if (output.Data != null)
                    return output.Data.ErrorMessage;
                else
                    return "خطا در اجرای سرویس دهنده سازمان ثبت رخ داد";
            }
            else
                return output.message.First();

        }
        public async Task<string> UnArchiveEstateInquiryInSabtTerminalServer(CancellationToken cancellationToken)
        {
            if (EstateInquiry == null) return "استعلام مرتبط مشخص نیست";
            var input = new DataTransferReceiveServiceInput();
            input.ReceiverCmsorganizationId = estateInquiryScriptoriumOrganization.LegacyId;
            ...
            input.EntityList = new List<EntityData>()
            {
                 new EntityData()
                 {
                      ...
                      FieldValues=new List<FieldValue>()
                      {
                         new FieldValue(){ FieldName="SendingStatus", Value="3"},
                         new FieldValue(){ FieldName="Id", Value= string.IsNullOrWhiteSpace(EstateInquiry.LegacyId) ? EstateInquiry.Id.ToString().Replace("-","").Replace("_","").ToUpper() : EstateInquiry.LegacyId }
                      }
                 }
            };
        ...
        {
            if (EstateInquiry == null) return "استعلام مرتبط مشخص نیست";
            DataTransferSendServiceInput input = new();
            var inquiryPerson = EstateInquiry.EstateInquiryPeople.FirstOrDefault();
            ...
            input.TheEntities.Add(new EntityInfo() { CommandType = EstateInquiry.WorkflowStatesId == EstateConstant.EstateInquiryStates.EditAndReSend ? 2 : 1, EntityId = !string.IsNullOrWhiteSpace(EstateInquiry.LegacyId) ? EstateInquiry.LegacyId : EstateInquiry.Id.ToString().Replace("-", "").Replace("_", "").ToUpper(), EntitySystemObjectId = "e7fc514ae1c441d88665e058b143d591", OrderNo = 1 });
            if (inquiryPerson != null)
            {
                input.TheEntities.Add(new EntityInfo() { CommandType = 1, EntityId = inquiryPerson.Id.ToString().Replace("-", "").Replace("_", "").ToUpper(), EntitySystemObjectId = "969ee03cb55f40819dbf011bf83b8767", OrderNo = 2 });
                if (inquiryPerson.PersonType == EstateConstant.PersonTypeConstant.RealPerson)
                    input.TheEntities.Add(new EntityInfo() { CommandType = 1, EntityId = inquiryPerson.Id.ToString().Replace("-", "").Replace("_", "").ToUpper(), EntitySystemObjectId = "8c72521b389f4503b43397a684ab50e9", OrderNo = 3 });
                else
                    input.TheEntities.Add(new EntityInfo() { CommandType = 1, EntityId = inquiryPerson.Id.ToString().Replace("-", "").Replace("_", "").ToUpper(), EntitySystemObjectId = "e0369b929c6b490f9d9322b866a1d5a6", OrderNo = 3 });
            ...
            if (result.IsSuccess)
            {
                if (result.Data.Successful)
                    ...
                    return "خطا در ارسال استعلام به سامانه املاک :" + result.Data.ErrorMessage;
            }
            else
                return "خطا در ارسال استعلام به سامانه املاک :" + result.message.FirstOrDefault();
        ...
        public async Task<string> SendDealSummaryToSabtTerminalServerByService(CancellationToken cancellationToken)
        {
            if (DealSummary == null) return "خلاصه معامله مرتبط مشخص نیست";
            DataTransferReceiveServiceInput input = new();
            input.ReceiverCmsorganizationId = dealSummaryScriptoriumOrganization.LegacyId;
        ...
        public async Task<string> SendDealSummaryToSabtEstateSystemByService(CancellationToken cancellationToken)
        {
            ...
            input.TheEntities.Add(new EntityInfo() { CommandType = 2, EntityId = !string.IsNullOrWhiteSpace(DealSummary.LegacyId) ? DealSummary.LegacyId : DealSummary.Id.ToString().Replace("_", "").Replace("_", "").ToUpper(), EntitySystemObjectId = "966dbae1f434485fb6e5e83f9b4dad01", OrderNo = 1 });
            ...
            if (result.IsSuccess)
            {
                if (result.Data.Successful)
                    ...
                    return "خطا در ارسال خلاصه معامله به سامانه املاک :" + result.Data.ErrorMessage;
            }
            else
                return "خطا در ارسال خلاصه معامله به سامانه املاک :" + result.message.FirstOrDefault();
        ...
        //private async Task<DownloadAttachmentViewModel> GetTaxInquiryFileFromArchive(Guid taxInquiryFileId, string mediaId, CancellationToken cancellationToken)
        //{

        //    var baseInfoUrl = _configuration.GetValue<string>("InternalGatewayUrl") + "Media/";
        //    var request = new SharedKernel.Contracts.HttpEndPointCaller.HttpEndpointRequest<DownloadAttachmentQuery>(baseInfoUrl + "Download", _userService.UserApplicationContext.Token,
        //        new DownloadAttachmentQuery()
        //        {
        //            AttachmentClientId = "9007",
        //            AttachmentRelatedRecordId = taxInquiryFileId.ToString(),
        //            AttachmentTypeId = "0910",
        //            AttachmentFileId = mediaId
        //        });

        //    return await _httpEndPointCaller.CallExternalPostApiAsync<Notary.SSAA.BO.DataTransferObject.ViewModels.Estate.MediaService.DownloadAttachmentViewModel, DownloadAttachmentQuery>(request, cancellationToken);
        ...
        public async Task<string> DeleteEstateInquiryInSabtTerminalServerByService(CancellationToken cancellationToken)
        {
            string str = "";
            var input = GetDeleteEstateInquiryCommands();
            var result = await _mediator.Send(input, cancellationToken);
            if (result.IsSuccess)
            {
                if (!result.Data.Successful)
        ...
        public async Task<string> RollbackEstateInquirySendStatusInSabtTerminalServerByService(CancellationToken cancellationToken)
        {
            string str = "";
            string state = null;
            if (EstateInquiry == null) return "استعلام مرتبط مشخص نیست";
            if (EstateInquiry.WorkflowStatesId == EstateConstant.EstateInquiryStates.NeedCorrection)
                state = "8";
            var input = RollbackEstateInquirySendStatusCommands(state);
            var result = await _mediator.Send(input, cancellationToken);
            if (result.IsSuccess)
            {
                if (!result.Data.Successful)
