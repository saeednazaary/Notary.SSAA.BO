...
namespace Notary.SSAA.BO.CommandHandler.Base
{
           ...
           where TRequest : BaseCommandRequest<TResult> where TResult : ApiResult, new()
    {
        ...
        protected BaseCommandHandler(IMediator mediator, IUserService userService, /*IAccountService accountService, IAuditLogRepository auditLogepository,*/
                                         ILogger logger)
        {
    ...
    public abstract class BaseExternalCommandHandler<TRequest, TResult> : IRequestHandler<TRequest, TResult>
           where TRequest : BaseExternalCommandRequest<TResult> where TResult : ExternalApiResult, new()
    {
        protected readonly IMediator _mediator;
        protected IUserService _userService;
        ...
        protected BaseExternalCommandHandler(IMediator mediator, IUserService userService, /*IAccountService accountService, IAuditLogRepository auditLogepository,*/
                                         ILogger logger)
        {
            _mediator = mediator ?? throw new ArgumentNullException(nameof(userService), "MediatR is Null"); ;
            this._userService = userService ?? throw new ArgumentNullException(nameof(userService), "User Serivce is Null");
