...
namespace Notary.SSAA.BO.QueryHandler.Estate.DealSummary
{
    public class GetDealSummaryByIdQueryHandler : BaseQueryHandler<GetDealSummaryByIdQuery, ApiResult<DealSummaryViewModel>>
    {
        ...
        public async Task<GetGeolocationByIdViewModel> GetGeoLocations(Domain.Entities.DealSummary dealSummary, CancellationToken cancellationToken)
        {
            ...
            list.AddRange(dealSummary.DealSummaryPeople.Where(x => x.IssuePlaceId.HasValue).Select(x => x.IssuePlaceId.Value).ToList());
            list.AddRange(dealSummary.DealSummaryPeople.Where(x => x.CityId.HasValue).Select(x => x.CityId.Value).ToList());
            list.AddRange(dealSummary.DealSummaryPeople.Where(x => x.BirthPlaceId.HasValue).Select(x => x.BirthPlaceId.Value).ToList());
            list.AddRange(dealSummary.DealSummaryPeople.Where(x => x.NationalityId.HasValue).Select(x => x.NationalityId.Value).ToList());
            if (dealSummary.EstateInquiry.GeoLocationId.HasValue)
                list.Add(dealSummary.EstateInquiry.GeoLocationId.Value);
