...
{
    internal class DealSummaryVerificationQueryHandler : BaseQueryHandler<DealSummaryVerificationQuery<DealSummaryVerificationResultViewModel>, ApiResult<DealSummaryVerificationResultViewModel>>
    {
        ...
        //string senderNotaryCmsId = "";
        public DealSummaryVerificationQueryHandler(IMediator mediator, IUserService userService, ILogger logger,
            IHttpEndPointCaller httpEndPointCaller, IConfiguration configuration, IEstateInquiryRepository estateInquiryRepository, IDealSummaryTransferTypeRepository dealSummaryTransferTypeRepository, IDealSummaryRepository dealSummaryRepository, IRepository<Domain.Entities.DocumentEstateInquiry> documentEstateInquiryRepository, IDateTimeService dateTimeService) : base(mediator, userService)
        {
            ...
            this._baseInfoServiceHelper = new BaseInfoServiceHelper(mediator);
        }

        protected async override Task<ApiResult<DealSummaryVerificationResultViewModel>> RunAsync(DealSummaryVerificationQuery<DealSummaryVerificationResultViewModel> input, CancellationToken cancellationToken)
        {
            ...
            {
                // RaiseError("در لیست ورودی بیش از یک خلاصه معامله مالکیت بروی یک استعلام ثبت شده است", -21);
            ...
            foreach (DSUDealSummaryObject deal in input.DsuDealSummary)
            {
                ...
                if (await MandatoryFieldsValidation(deal,  cancellationToken))
                {
                    ...
                    {
                        if (await DealSummaryInquiryValidation(deal, deal.ESTEstateInquiryId.Trim(), cancellationToken, false, input.Tag))
                        {
        ...
        protected override bool HasAccess(DealSummaryVerificationQuery<DealSummaryVerificationResultViewModel> request, IList<string> userRoles)
        {
        ...
        public async Task<bool> MandatoryFieldsValidation(DSUDealSummaryObject dealsummary,  CancellationToken cancellationToken)
        {
            ...
            //var TransferType = await _dealSummaryTransferTypeRepository.GetByIdAsync(cancellationToken, dealsummary.DSUTransferTypeId);
            if (dealsummary.TheDSUTransferType.IsRestricted == "1" && dealsummary.TheDSUTransferType.Code != "239")
            {
        ...
        public async Task<bool> DealSummaryInquiryValidation(DSUDealSummaryObject dealsummary, string inquiryId,  CancellationToken cancellationToken, bool tag = false, object flag = null)
        {
            ...
            //string sql = string.Format(@"select e.*,g.id as notarygeo,g1.id as regunitgeo from estestateinquiry e inner join cmsorganization org on e.producerinquiryid=org.id inner join cmsorganization cmsunit on e.selfinquiryid=cmsunit.id inner join scriptorium s on org.scriptoriumid=s.id inner join geolocation g on s.geolocationid=g.id inner join unit u on cmsunit.unitid=u.id inner join geolocation g1 on u.geolocationid=g1.id where e.id='{0}'", inquiryId);
            //System.Data.DataTable dt = ExecuteQuery(sql, dealsummary.DealSummeryIssuerId);
            ...
            if (result)
            {
                ...
                foreach (DSURealLegalPersonObject rlp in dealsummary.TheDSURealLegalPersonList)
                {
                    ...
                    if (rlp.DSURelationKindId == "102")
                    {
                        ...
                        //    {
                        //        result = false;
                        //        errorMessages.Add("جز سهم و کل سهم ذینفع نمی تواند خالی باشد");
                        //        break;
                        //    }
                        //    else if (rlp.SharePart.Value == 0 || rlp.ShareTotal.Value == 0)
                        //    {
                        //        result = false;
                        //        errorMessages.Add("جز سهم و کل سهم ذینفع نمی تواند صفر  باشد");
                        //        break;
                        //    }
                        //    else
                        //        if (rlp.SharePart.Value > rlp.ShareTotal.Value)
                        //        {
                        //            result = false;
                        //            errorMessages.Add("جز سهم ذینفع بیشتر از کل سهم آن است");
                        //            break;
                        //        }
                        //}
                        //else
            ...
            if (estateInquiry != null)
            {
                ...
                if (estateInquiryList.Count > 0)
                {
                    ...
                    if (dealSummaryList.Count > 0)
                    {
                        ...
                        //senderNotaryCmsId = dealSummaryList[0].ScriptoriumId.ToString();
                        var scriptoriumInfo = await _baseInfoServiceHelper.GetScriptoriumById(new string[] { dealSummaryList[0].ScriptoriumId }, cancellationToken);
                        notaryOfficeName = scriptoriumInfo.ScriptoriumList[0].Name;
                    }
                }
            }
            return result;
