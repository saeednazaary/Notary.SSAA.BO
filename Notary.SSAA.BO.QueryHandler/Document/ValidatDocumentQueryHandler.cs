...
{
    public class ValidatDocumentQueryHandler : BaseQueryHandler<ValidatDocumentQuery, ApiResult<ValidatDocumentViewModel>>
    {
        private ValidatorGateway _validatorGateway;
        public ValidatDocumentQueryHandler(IMediator mediator, IUserService userService, ValidatorGateway validatorGateway) : base(mediator, userService)
        {
            _validatorGateway = validatorGateway;
        }

        protected override bool HasAccess(ValidatDocumentQuery request, IList<string> userRoles)
        {
            return userRoles.Contains(RoleConstants.Sardaftar) || userRoles.Contains(RoleConstants.Daftaryar) || userRoles.Contains(RoleConstants.SanadNevis);
        }

        protected override async Task<ApiResult<ValidatDocumentViewModel>> RunAsync(ValidatDocumentQuery request, CancellationToken cancellationToken)
        {
            string response = "";
            ApiResult<ValidatDocumentViewModel> apiResult = new();
            ValidatDocumentViewModel Result = new ValidatDocumentViewModel();
