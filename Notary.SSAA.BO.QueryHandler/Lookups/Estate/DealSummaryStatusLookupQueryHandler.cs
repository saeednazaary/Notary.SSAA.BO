...
{
    public class DealSummaryStatusLookupQueryHandler : BaseQueryHandler<DealSummaryStatusLookupQuery, ApiResult<DealSummaryStatusLookupViewModel>>
    {
        private readonly IWorkfolwStateRepository _workfolwStateRepository;

        public DealSummaryStatusLookupQueryHandler(IMediator mediator, IUserService userService,
        ...
        protected override bool HasAccess(DealSummaryStatusLookupQuery request, IList<string> userRoles)
        {
            return userRoles.Contains(RoleConstants.Sardaftar) || userRoles.Contains(RoleConstants.Daftaryar) || userRoles.Contains(RoleConstants.SanadNevis);
        }

        protected override async Task<ApiResult<DealSummaryStatusLookupViewModel>> RunAsync(DealSummaryStatusLookupQuery request, CancellationToken cancellationToken)
        {
            DealSummaryStatusLookupViewModel result = new();
            ...
            var resultTemp = await _workfolwStateRepository.GetDealSummaryStatusLookupItems(request.PageIndex, request.PageSize, request.GridFilterInput, request.GlobalSearch, gridSortInput, request.SelectedItems, FieldsNotInFilterSearch,
            ...
            return new ApiResult<DealSummaryStatusLookupViewModel>(true, ApiResultStatusCode.Success, result, new List<string> { SystemMessagesConstant.Operation_Successful });
        }
    }
}
