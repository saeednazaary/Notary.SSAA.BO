...
{
    public class UsableEstateInquiryLookupQueryHandler : BaseQueryHandler<UsableEstateInquiryLookupQuery, ApiResult<EstateInquiryGrid2>>
    {
        ...
        public UsableEstateInquiryLookupQueryHandler(IMediator mediator, IUserService userService,
        ...
        protected override bool HasAccess(UsableEstateInquiryLookupQuery request, IList<string> userRoles)
        {
            return userRoles.Contains(RoleConstants.Sardaftar) || userRoles.Contains(RoleConstants.Daftaryar) || userRoles.Contains(RoleConstants.SanadNevis);
        }

        protected override async Task<ApiResult<EstateInquiryGrid2>> RunAsync(UsableEstateInquiryLookupQuery request, CancellationToken cancellationToken)
        {
            ...
            if (result.TotalCount > 0)
            {
                ...
                IEnumerable<EstateInquiryGridItem> resultItemList = new List<EstateInquiryGridItem>();



                var ForDealSummaryItemList = estateInquiryGridItemList.Where(x => (x.ResponseResult == "True" && x.InquiryResponseDate.CompareTo(persianFromDate1) >= 0 && x.InquiryResponseDate.CompareTo(persianToDate) <= 0 && x.ScriptoriumGeolocationId == x.UnitGeolocationId && !specificStatusList.Contains(x.SpecificStatus))
                                                    || (x.InquiryResponseDate.CompareTo(persianFromDate1) >= 0 && x.InquiryResponseDate.CompareTo(persianToDate) <= 0 && x.ScriptoriumGeolocationId == x.UnitGeolocationId && ((string.IsNullOrWhiteSpace(x.SpecificStatus) && x.ResponseResult == "True") || x.SpecificStatus == EstateConstant.EstateInquirySpecificStatus.Arrested) && x.IsExecutiveTransfer)
                                                    || (x.ResponseResult == "True" && x.InquiryResponseDate.CompareTo(persianFromDate2) >= 0 && x.InquiryResponseDate.CompareTo(persianToDate) <= 0 && x.ScriptoriumGeolocationId != x.UnitGeolocationId && !specificStatusList.Contains(x.SpecificStatus))
                                                    || (x.InquiryResponseDate.CompareTo(persianFromDate2) >= 0 && x.InquiryResponseDate.CompareTo(persianToDate) <= 0 && x.ScriptoriumGeolocationId != x.UnitGeolocationId && ((string.IsNullOrWhiteSpace(x.SpecificStatus) && x.ResponseResult == "True") || x.SpecificStatus == EstateConstant.EstateInquirySpecificStatus.Arrested) && x.IsExecutiveTransfer)
                                                    );

                

                string[] specificStatusList1 = new string[] {
                    EstateConstant.EstateInquirySpecificStatus.OnlyAllowedForDivision,
                    EstateConstant.EstateInquirySpecificStatus.AllowedForDivision};

                var ForDivisionRequestItemList = estateInquiryGridItemList.Where(x => (x.ResponseResult == "True" && string.IsNullOrWhiteSpace(x.AttachedToDealSummary) && x.InquiryResponseDate.CompareTo(persianFromDate1) >= 0 && x.InquiryResponseDate.CompareTo(persianToDate) <= 0 && x.ScriptoriumGeolocationId == x.UnitGeolocationId && specificStatusList1.Contains(x.SpecificStatus))
                                                 ...
                                                 || (x.ResponseResult == "True" && string.IsNullOrWhiteSpace(x.AttachedToDealSummary) && x.InquiryResponseDate.CompareTo(persianFromDate2) >= 0 && x.InquiryResponseDate.CompareTo(persianToDate) <= 0 && x.ScriptoriumGeolocationId != x.UnitGeolocationId && specificStatusList1.Contains(x.SpecificStatus))
                ...
                resultItemList = ForDealSummaryItemList.Union(ForDivisionRequestItemList);
            ...
            return new ApiResult<EstateInquiryGrid2>(true, ApiResultStatusCode.Success, r, new List<string> { "عملیات با موفقیت انجام شد" });
        }
    }
}
