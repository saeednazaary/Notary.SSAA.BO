...
{
    public class EstateInquiryForDealSummaryLookupQueryHandler : BaseQueryHandler<EstateInquiryForDealSummaryLookupQuery, ApiResult<EstateInquiryGrid2>>
    {
        ...
        public EstateInquiryForDealSummaryLookupQueryHandler(IMediator mediator, IUserService userService,
        ...
        protected override bool HasAccess(EstateInquiryForDealSummaryLookupQuery request, IList<string> userRoles)
        {
            return userRoles.Contains(RoleConstants.Sardaftar) || userRoles.Contains(RoleConstants.Daftaryar) || userRoles.Contains(RoleConstants.SanadNevis);
        }

        protected override async Task<ApiResult<EstateInquiryGrid2>> RunAsync(EstateInquiryForDealSummaryLookupQuery request, CancellationToken cancellationToken)
        {
            ...
            if (result.TotalCount > 0)
            {
                ...
                IEnumerable<EstateInquiryGridItem> resultItemList = null;
                if (request.ExtraParams.IsRestrictedDealSummary)
                {
                    resultItemList = estateInquiryGridItemList.Where(x => (x.ResponseResult == "True" && x.InquiryResponseDate.CompareTo(persianFromDate1) >= 0 && x.InquiryResponseDate.CompareTo(persianToDate) <= 0 && x.ScriptoriumGeolocationId == x.UnitGeolocationId && !specificStatusList.Contains(x.SpecificStatus))
                ...
                result.GridItems = resultItemList.Skip((request.PageIndex - 1) * request.PageSize).Take(request.PageSize).ToList();
            }
