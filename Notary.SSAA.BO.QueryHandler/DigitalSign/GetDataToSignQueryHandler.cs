                    ...
                    if (!string.IsNullOrWhiteSpace(mappingInfoDetail.NewPropertyName))
                    {
                        ...
                        else if (mappingInfoDetail.NewPropertyOwnerPath.StartsWith("BI:"))
                        {
                            ...
                            if (id != null)
                            {
                                ...
                                {
                                    var geo = await _baseInfoServiceHelper.GetGeoLocationById(new int[] { Convert.ToInt32(id) }, cancellationToken);
                        ...
                        else if (mappingInfoDetail.NewPropertyOwnerPath.StartsWith("TB:"))
                        {
                            ...
                            if (id != null)
                            {
                                ...
                                {
                                    var estateSection = await EstateSectionRepository.TableNoTracking.Where(x => x.Id == id.ToString()).FirstOrDefaultAsync(cancellationToken);
                                ...
                                {
                                    var estateSubSection = await EstateSubSectionRepository.TableNoTracking.Where(x => x.Id == id.ToString()).FirstOrDefaultAsync(cancellationToken);
