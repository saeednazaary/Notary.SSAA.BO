...
{
    public class GetSignRequestByIdAdminQueryHandler : BaseQueryHandler<GetSignRequestAdminByIdQuery, ApiResult<SignRequestViewModel>>
    {
        private readonly ISignRequestRepository _signRequestRepository;

        public GetSignRequestByIdAdminQueryHandler(IMediator mediator, IUserService userService, ISignRequestRepository signRequestRepository)
            : base(mediator, userService)
        {
            _signRequestRepository = signRequestRepository ?? throw new ArgumentNullException(nameof(signRequestRepository));

        }
        protected override bool HasAccess(GetSignRequestAdminByIdQuery request, IList<string> userRoles)
        {
            return userRoles.Contains(RoleConstants.SSAAAdmin);
        }

        protected override async Task<ApiResult<SignRequestViewModel>> RunAsync(GetSignRequestAdminByIdQuery request, CancellationToken cancellationToken)
        {

            SignRequestViewModel result = new();
            ApiResult<SignRequestViewModel> apiResult = new();
            Domain.Entities.SignRequest signRequest = await _signRequestRepository.GetKatebSignRequesById(request.SignRequestId
                , cancellationToken);
            if (signRequest != null)
            {
                ...
                result.SignRequestId = signRequest.Id.ToString();
                ...
                foreach (SignRequestPersonViewModel item in result.SignRequestPersons)
                {
                    if (personalImages?.IsSuccess == true)
                    {
                        ...
                        item.PersonalImage = personalImage == null ? null : Convert.ToBase64String(personalImage.PersonalImage);
                ...
                result.SignRequestPersons = result.SignRequestPersons.OrderBy(x => x.RowNo).ToList();
                result.SignRequestRelatedPersons = result.SignRequestRelatedPersons.OrderBy(x => x.RowNo).ToList();
